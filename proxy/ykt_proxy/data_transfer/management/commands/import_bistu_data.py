...
BISTU_SERVER = "http://127.0.0.1:8080/data/bistu"
BISTU_UNI_INFO_DICT = {
    "university_name": "北京信息科技大学",
    "university_province": "北京市",
    "university_city": "北京市",
    "university_address": "北京市",
    "university_url": "https://xust.yuketang.cn/",
    "current_term": "201902",
    "university_prefix": "bistu",
...
def generate_data():
    ...
    department_data = send_request(url='{}/get_department_data'.format(BISTU_SERVER))

    print 'tra_classroom_data......'
    tra_classroom_data = send_request(url='{}/get_tra_classroom_data'.format(BISTU_SERVER))

    print 'user_data......'
    user_data = send_request(url='{}/get_user_data'.format(BISTU_SERVER))

    print 'course_data......'
    course_data = send_request(url='{}/get_course_data'.format(BISTU_SERVER), params=params)

    print 'choose_data......'
    choose_data = send_request(url='{}/get_choose_data'.format(BISTU_SERVER), params=params)

    department_map = {d["department_code"]: [d["department_name"]] for d in department_data}
    ...
    for info_dict in tra_classroom_data:
        ...
        department_name = department_map.get(info_dict.get('department_code'), [""])[0]
    ...
    # ---------------------------------------------------------------------------------
    tmp_csv_file_path = '/tmp/university_data/{}'.format(BISTU_UNI_INFO_DICT.get('university_name'))
    result_info_dict = {
        "department": department_map.values(),
